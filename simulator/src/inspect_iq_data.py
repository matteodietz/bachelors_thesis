...
# Import your data loaders and virtual AFE function
...
if __name__ == '__main__':
    ...
    # --- 3. Define Test Cases ---
    num_random_channels = 9
    num_angles_per_channel = 3
    ...
    all_max_errors_real, all_avg_errors_real, all_norm_errors_real = [], [], []
    all_max_errors_imag, all_avg_errors_imag, all_norm_errors_imag = [], [], []
    ...
    for i, config in enumerate(test_configs):
        ...
        # --- Generate Your High-Fidelity I/Q Data for this case ---
        ...
        # Save the data for the VERY LAST case for plotting
    ...
    # --- 6. Plotting (for the fixed, final case) ---
    my_envelope_plot = np.abs(my_iq_aline_plot)
    gt_envelope_plot = np.abs(gt_iq_aline_plot)
    
    plt.figure(figsize=(14, 7))
    plt.plot(my_time_axis_plot * 1e6, my_iq_aline_plot.real, 'b-', linewidth=0.5, label='Generated I Component')
    plt.plot(my_time_axis_plot * 1e6, my_iq_aline_plot.imag, 'r-', linewidth=0.5, label='Generated Q Component')
    plt.plot(my_time_axis_plot * 1e6, my_envelope_plot, 'k-', linewidth=1.5, label='Generated Envelope')
    
    plt.title(f'Your Generated I/Q Data (Channel {channel_for_plot}, Angle {angle_for_plot})')
    plt.xlabel('Time (µs)')
    plt.ylabel('Amplitude')
    plt.legend()
    plt.grid(True)
    plt.suptitle("Figure 1: Your Generated Data (Fixed Test Case)", fontsize=16)

    plt.figure(figsize=(14, 7))
    plt.plot(gt_time_axis_plot * 1e6, gt_iq_aline_plot.real, 'b-', linewidth=0.5, label='PICMUS I Component')
    plt.plot(gt_time_axis_plot * 1e6, gt_iq_aline_plot.imag, 'r-', linewidth=0.5, label='PICMUS Q Component')
    plt.plot(gt_time_axis_plot * 1e6, gt_envelope_plot, 'k-', linewidth=1.5, label='PICMUS Envelope')
    
    plt.title(f'PICMUS Ground Truth I/Q Data (Channel {channel_for_plot}, Angle {angle_for_plot})')
    plt.xlabel('Time (µs)')
    plt.ylabel('Amplitude')
    plt.legend()
    plt.grid(True)
    plt.suptitle("Figure 2: Ground Truth Data (Fixed Test Case)", fontsize=16)
